...
export default function Blog({ blogPosts }: { blogPosts: Blog[] }) {
  ...
  return (
    <section>
      <h1 className="mb-4 text-xl font-bold sm:text-3xl ">Blog</h1>
      <div className="grid grid-cols-1 gap-4">
        {blogPosts.map((blogPost) => (
          ...
          >
            <div className="grid h-full grid-cols-1 p-4 rounded-lg bg-zinc-50/95 dark:bg-zinc-800/95">
              <div className="grid ml-4">
                <h3 className="mb-2 text-lg font-bold sm:text-xl ">
                  {blogPost.title}
                </h3>
                <p className="mb-2 dark:text-white">{blogPost.summary}</p>
                <span className="mb-2 place-self-end dark:text-white">
